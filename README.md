...
When `mem0.enabled` is **false**, NanoBot uses its built-in, token-based `MemoryConsolidator` for session history and treats `workspace/memory/MEMORY.md`/`HISTORY.md` as read-only compatibility inputs.
When `mem0.enabled` is **true**, NanoBot:

- Disables internal consolidation and delegates durable memory to Mem0 via MCP (auto-recall / auto-capture).
- Keeps `MEMORY.md`/`HISTORY.md` read-only; built-in file tools refuse writes/edits to those files.

See [docs/mem0.md](docs/mem0.md) for backup, restore, sharing Mem0 data, and details on how it interacts with NanoBot's own memory model.
