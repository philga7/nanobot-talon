...
class WriteFileTool(_FsTool):
    ...
    def _is_memory_file(self, resolved: Path) -> bool:
        """Return True if path is MEMORY.md or HISTORY.md under memory/."""
        if not self._workspace:
            return False
        try:
            rel = resolved.resolve().relative_to(Path(self._workspace).resolve())
        except ValueError:
            return False
        return str(rel) in self._MEMORY_FILES or rel.as_posix() in self._MEMORY_FILES
    ...
    async def execute(self, path: str, content: str, **kwargs: Any) -> str:
        try:
            fp = self._resolve(path)
            if self._block_memory_files and self._is_memory_file(fp):
                return (
